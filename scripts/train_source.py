#!/usr/bin/env python3
"""Training script for transfer reinforcement learning."""
...
class TransferLearningTrainer:
    """Trainer for transfer reinforcement learning experiments."""
    ...
    def train(self):
        ...
        self.logger.info(f"Environment: {self.config['env']['name']}")
        ...
        for episode in range(self.config["training"]["total_episodes"]):
            ...
            self.training_stats["exploration_rates"].append(self.agent.exploration_rate)
    ...
    def _generate_plots(self):
        ...
        if self.training_stats["eval_rewards"]:
            eval_episodes = np.arange(
                ...
                len(self.training_stats["episode_rewards"]) + 1,
                self.config["training"]["eval_frequency"]
            )
        ...
        plt.subplot(2, 2, 4)
        plt.plot(self.training_stats["exploration_rates"])
        ...
        plt.tight_layout()
        plt.savefig(plots_path / "training_curves.png", dpi=300, bbox_inches="tight")
        plt.close()
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Train transfer learning agent")
    ...
    # Enable transfer learning if requested
    if args.transfer:
    ...
    # Load source model for transfer if enabled
    if config["transfer"]["enabled"] and "source_model_path" in config["transfer"]:
        source_model_path = config["transfer"]["source_model_path"]
        if Path(source_model_path).exists():
            # Create temporary source agent to load weights
            source_agent = QLearningAgent(
                action_space_size=trainer.env.action_space.n,
                use_neural_network=config["agent"].get("use_neural_network", False)
            )
            source_agent.load(source_model_path)
            
            # Transfer knowledge
            trainer.agent.transfer_knowledge(
                source_agent, 
                config["transfer"]["transfer_type"]
            )
            trainer.logger.info(f"Transferred knowledge from {source_model_path}")
        else:
            trainer.logger.warning(f"Source model not found: {source_model_path}")
